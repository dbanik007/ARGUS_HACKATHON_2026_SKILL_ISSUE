...
export class DashboardComponent implements OnInit, AfterViewChecked {
  ...
  getVerdictMarkdown(): string {
    ...
    md += `> ${session.final_verdict === 'GO' ? 'Consensus reached. Roster audits validated. Proceed under baseline parameters.' :
               session.final_verdict === 'NEGOTIATE' ? 'Initial criteria rejected. Re-negotiating contract parameters. Propose modified terms.' :
  ...
  exportVerdictMarkdown(): void {
    ...
    const safeName = tenderName.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
