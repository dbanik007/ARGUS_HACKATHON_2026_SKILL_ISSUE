...
const EVAL_SESSIONS = [
  {
    ...
    messages: [
      ...
      { sender: 'Board of Directors', text: 'The Board has reviewed all departmental assessments for the Next-Gen Medical Telemetry Suite tender. All seven agents — Sales, Resource, Technical Architecture, Risk, Operations, Legal, and Finance — have validated project parameters within their respective remits. HIPAA staffing is confirmed, financial margins are strong, and strategic fit is clear. The Board issues its unanimous verdict.\n[VERDICT: GO]', round: 2 },
    ],
  },
  {
  ...
  },
  {
    ...
    timeline_months: 12,
    industry: 'E-Commerce',
    roster: ['Account Executive', 'Technical Architect', 'Risk Analyst', 'Operations Manager', 'Resource', 'Financial'],
    final_verdict: 'GO',
    final_budget: 1500000,
    messages: [
      { sender: 'Account Executive', text: 'ShopSphere Global is one of the fastest-growing E-Commerce platforms in the region. The $1.5M, 12-month engagement covers a full omnichannel intelligence stack — exactly our sweet spot. Domain match with our RetailX and DataPulse track record is strong. This is a GO.', round: 1 },
      ...
      { sender: 'Board of Directors', text: 'The Board has reviewed all assessments for the ShopSphere Global Omnichannel Platform tender. All six participating agents — Sales, Resource, Technical Architecture, Risk, Operations, and Finance — have validated the engagement. Domain alignment is exceptional, financial margins are strong, and delivery risk is low. The Board issues a unanimous verdict.\n[VERDICT: GO]', round: 2 },
    ],
  },
  {
    ...
    final_budget: 420000,
    messages: [
      { sender: 'Account Executive', text: 'CyberShield is a prestigious defense-sector client and this $420K engagement would open the national security vertical for us. 6 months is tight but doable. I advocate for GO — the reputational upside alone justifies this.', round: 1 },
      ...
      { sender: 'Board of Directors', text: 'The Board has reviewed all assessments for the CyberShield Defense Corp tender. Six out of six departments have flagged critical blockers: staffing gap at 60% below requirement, negative financial margin, ITAR legal obligation, and a timeline at half the viable minimum. Despite the strategic appeal of the defense vertical, the Board cannot approve an engagement with this risk profile. The Board issues its verdict.\n[VERDICT: NO-GO]', round: 2 },
    ],
  },
  {
    ...
    final_budget: 680000,
    messages: [
      { sender: 'Account Executive', text: 'FastTrack Logistics is a rapidly scaling last-mile player and their optimization engine project is a well-defined, contained scope. $680K over 8 months is realistic and aligns with our Logistics track record from the LogiRoute engagement. Strong advocate for GO.', round: 1 },
      ...
      { sender: 'Board of Directors', text: 'The Board has reviewed the FastTrack Logistics tender. All six agents have validated the engagement. Domain alignment, financial margins, and staffing are all green. Ada Lovelace\'s July leave is a noted operational flag that Operations must manage within sprint planning. The Board approves this engagement.\n[VERDICT: GO]', round: 2 },
    ],
  },
];
...
async function seed() {
  ...
  try {
    ...
    for (let i = 0; i < EVAL_SESSIONS.length; i++) {
      ...
      // Idempotency: skip if a session with this tender_name + user already exists
      const exists = await client.query(
        `SELECT id FROM evaluation_sessions WHERE tender_name = $1 AND user_id = $2`,
        [s.tender_name, userId]
      );
      if (exists.rows.length > 0) {
        console.log(`   ~ Session already exists: "${s.tender_name}" (user ${ownerEmail})`);
        continue;
      }
